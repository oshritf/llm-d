...
5. [Multi Model Serving](./multi-model-serving/README.md) - Allows to deploy multiple large language models (LLMs) in a cluster to support different workloads. Additionally, each base model may have multiple Low-Rank Adaptations (LoRAs). For model aware routing, the Body Based Routing (BBR) extracts information such as the model name from the request body.
